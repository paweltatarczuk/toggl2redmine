...
class Toggl {
   ...
   * @param {Integer} pid Filter by project id.
   * @param {Callable} callback
   */
  fetchTimeEntries(startDate, endDate, group, wid, pid, callback) {
    let self = this;
    this.client.getTimeEntries(startDate, endDate,
      ...
      // Filter out entries by project
      if (pid) {
        timeEntries = timeEntries.filter(function(timeEntry) {
          return timeEntry.hasOwnProperty('pid') &&
                 timeEntry['pid'] == pid;
        });
      }
